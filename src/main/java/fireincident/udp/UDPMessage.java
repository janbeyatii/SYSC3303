...
public class UDPMessage {
    ...
    private final udp.MessageType type;
    private final String[]    fields;

    public UDPMessage(udp.MessageType type, String... fields) {
        this.type   = type;
        this.fields = fields;
    }
    public static UDPMessage incidentReport(Incident inc) {
        return new UDPMessage(udp.MessageType.INCIDENT_REPORT,
                inc.getTime(),
    ...
    public static UDPMessage dispatchDrone(int droneId, Incident inc) {
        return new UDPMessage(udp.MessageType.DISPATCH_DRONE,
    ...
    public static UDPMessage droneArrived(int droneId, Incident inc) {
        return new UDPMessage(udp.MessageType.DRONE_ARRIVED,
                String.valueOf(droneId),
                inc.getKey());
    }
    public static UDPMessage droneDroppedAgent(int droneId, Incident inc) {
        return new UDPMessage(udp.MessageType.DRONE_DROPPED_AGENT,
                String.valueOf(droneId),
                inc.getKey());
    }
    public static UDPMessage droneReturning(int droneId) {
        return new UDPMessage(udp.MessageType.DRONE_RETURNING,
                String.valueOf(droneId));
    }
    public static UDPMessage droneIdle(int droneId) {
        return new UDPMessage(udp.MessageType.DRONE_IDLE,
                String.valueOf(droneId));
    }
    public static UDPMessage droneState(int droneId, String state, Integer zoneId) {
        return new UDPMessage(udp.MessageType.DRONE_STATE,
                String.valueOf(droneId),
                state,
                zoneId == null ? "" : String.valueOf(zoneId));
    }
    public static UDPMessage incidentCompleted(Incident inc) {
        return new UDPMessage(udp.MessageType.INCIDENT_COMPLETED, inc.getKey());
    }
    public static UDPMessage shutdown() {
        return new UDPMessage(udp.MessageType.SHUTDOWN);
    }
    public byte[] toBytes() {
    ...
    public static UDPMessage fromString(String wire) {
        ...
        udp.MessageType type = udp.MessageType.valueOf(parts[0]);
    ...
    public udp.MessageType getType() { return type; }
